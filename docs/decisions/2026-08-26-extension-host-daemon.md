...
`start()`的场景——比如打开一个新面板——正是 `docs/decisions/2026-08-22-same-port-respawn-optimization.md`
	里"两个 hub 合法共存"那条已经点出来的场景:新面板会让它在一个新的临时端口上
	再起一个它自己知道的 hub。daemon 只回收自己本轮明确 spawn 且过宽限期的进程，
	不会根据瞬时 CDP 空引用去猜测旧 hub 是否可回收，避免启动窗口误杀真实服务。
	这条 `[LAUNCH ERROR]` 日志本身只是噪音,不代表出了新问题。
