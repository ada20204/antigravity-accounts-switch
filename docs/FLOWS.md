...
三种状态下的完整流程。设计取舍的**原因**一律见 `DECISIONS.md`,这里只讲"实际发生什么"。
...
> ⚠️ 已知未解:清掉前两处**通常**能让 Antigravity 显示登录页,但实测出现过清掉后仍是登录态的情况(见 DECISIONS.md)。说明可能还有第三处我们没找到的登录态来源。`finish` 因此不依赖任何文件是否存在来判断,只用 `connect` 能不能读到活动凭证。
